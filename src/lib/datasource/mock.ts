...
export const mockDs: DataSource = {
  ...
  async upsertProject(input: any) {
    ...
    if (input.id) {
      ...
      const updated = { ...store.projects[index], ...payload, updated_at: new Date().toISOString() };
      store.projects[index] = updated;
      persist();
      return { data: updated, error: null };
    } else {
      // Create new
      const row = buildProject(payload);
  ...
  async seedSampleDeals() {
    const samples = [
      { 
        ...
        stage: "analysis", 
        currency: "EUR", 
        kpis: { rooms: 24, location: "Ibiza, Spain" }
      },
      { 
        name: "Sample Deal B", 
        stage: "analysis", 
        currency: "EUR", 
        kpis: { rooms: 28, location: "Lagos, Portugal" }
      }
    ];
    
    samples.forEach(sample => {
      const row = buildProject(sample);
      store.projects.push(row);
    });
    persist();
    
    return { count: samples.length, error: null };
  },
